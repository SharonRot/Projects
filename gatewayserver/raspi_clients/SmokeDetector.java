...
public class SmokeDetector implements IotDevice {
	...
	private String currentMessage = "Info message : Smoke detector is working";  
	...
	public SmokeDetector(String serialNumber, String IP) {
	...
	private void startRecieveMessages() {
		RecieveMessages recieveMessages = new RecieveMessages();
	...
	private void initTcpConnection() {
		...
		try {
			address = new InetSocketAddress(InetAddress.getByName(IP), ServerPort.TCP_SERVER_PORT.getPort());
	...
	private void sendMessage(ByteBuffer bufferToSend, IotMessage iotMessage) {
		try {
			bufferToSend = ByteBuffer.wrap(createMessage(iotMessage));
		...
		public void run() {
			while(keepRunning) {
				try {
					...
					System.out.println("SmokeDetector received: " + response);
...
/*******************************************************************************/
	private String createJsonMessage(IotMessage iotMessage) {
		...
		System.out.println(outerJsonMessage.toString());
		return outerJsonMessage.toString();
	}
	
	public void createFireEvent() throws InterruptedException {
		currentUpdateType = "Critical";
		currentMessage = "Critical message: fire!!";
